...
@WebServlet("/materialRequestsEE")
public class MaterialRequestsEE extends HttpServlet{
	...
	try {
		...
		pst=con.prepareStatement("select fk_material_request_id from material_request_status where material_request_status_details='waiting for approval from ee'");
		rs=pst.executeQuery();
		while(rs.next()) {
								...
								+ "<td>"+linemanID+"</td>"
										+ "<td>"+requestID+"</td>"
												+ "<td>"+materialDetails+"</td>"
														+ "<td><a href='approveRequestEE?requestID="+requestID+"'>Approve</a></td></tr>");
			}
			rsNew.close();
