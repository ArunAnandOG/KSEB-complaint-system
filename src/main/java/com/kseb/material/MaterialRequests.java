...
@WebServlet("/materialRequests")
public class MaterialRequests extends HttpServlet{
	...
	try {
		...
		out.print("<body>");
		out.print("<h1>Material Requests</h1>");
		...
		out.print("<tr><th>Complaint ID</th>"
				...
				+ "<th>Approve Request</th></tr>");
		
		pst=con.prepareStatement("select fk_material_request_id from material_request_status where material_request_status_details='waiting for approval from material department'");
		rs=pst.executeQuery();
		while(rs.next()) {
			...
			while(rsNew.next()) {
				...
				rsNewx.close();
				out.print("<tr><td>"+complaintID+"</td>"
						+ "<td>"+complaintDetails+"</td>"
								+ "<td>"+linemanID+"</td>"
										+ "<td>"+requestID+"</td>"
												+ "<td>"+materialDetails+"</td>"
														+ "<td><a href='approveRequestMaterial?requestID="+requestID+"'>Approve</a></td></tr>");
			}
			rsNew.close();
