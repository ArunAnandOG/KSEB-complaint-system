...
public class ViewMyAllocations extends HttpServlet{
	...
	try {
		...
		while(rs.next()) {
			...
			pstNew=con.prepareStatement("select complaint_status from complaint_status where complaint_id_status=?");
			pstNew.setInt(1, complaintID);
			rsNew=pstNew.executeQuery();
			while(rsNew.next()) {
			...
			out.print("<tr><td>"+allocationID+"</td>"
					+ "<td>"+date+"</td>"
							+ "<td>"+complaintID+"</td>"
									+ "<td>"+name+"</td>"
											+ "<td>"+phone+"</td>"
													+ "<td>"+post+"</td>"
															+ "<td>"+complaint+"</td>"
																	+ "<td>"+status+"</td>"
																			+ "<td><a href='editStatusLineman?complaintID="+complaintID+"'>Edit</a></td></tr>");
		}
		rs.close();
